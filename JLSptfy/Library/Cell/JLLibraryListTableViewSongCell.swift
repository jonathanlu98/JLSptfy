//
//  JLLibraryListTableViewSongCell.swift
//  JLSptfy
//
...
class JLLibraryListTableViewSongCell: UITableViewCell {
...
//        titleLabel.type = .leftRight
    ...
    func setupUI() {
        ...
        self.subTitleLabel.text! = ((item.artists?.first?.name ?? "-")+" · "+(item.album?.name ?? "-")) 
        fetchImage(URL(string: item.album?.images?.last?.url ?? ""), imageView: self.iconImageView)
    }
