...
struct JLLibraryQuickJSON: Codable {
    let playlists: Search_PagingPlaylists?
    let albums: Library_PagingAlbums?
    let artists: Library_Following?
    let tracks: Library_PagingTracks?
...
// MARK: JLLibraryQuickJSON convenience initializers and mutators

extension JLLibraryQuickJSON {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(JLLibraryQuickJSON.self, from: data)
    }
    ...
    func with(
        albums: Library_PagingAlbums?? = nil,
        artists: Library_Following?? = nil,
        tracks: Library_PagingTracks?? = nil,
        playlists: Search_PagingPlaylists?? = nil,
        error: SPTJSONError?? = nil
    ) -> JLLibraryQuickJSON {
        return JLLibraryQuickJSON(
    ...
    func getLastFollowingID() -> String? {
        ...
        if (self.artists != nil) {
            return self.artists?.artists?.cursors?.after
        } else {
            return nil
        }
    }
